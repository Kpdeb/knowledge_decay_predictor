...
ML model: delegates to the ml-service via HTTP.
"""
...
# Spaced repetition intervals (days) — Ebbinghaus-inspired
...
def memory_strength(
    ...
    """
    Returns S (memory strength) in hours.
    Higher S → retention decays slower.
    """
    diff_factor = DIFFICULTY_FACTOR.get(difficulty, 1.0)
    score_factor = 1 + (score / 100)           # 1.0 – 2.0
    review_factor = 1 + (review_count * 0.3)   # boosts with repetitions
    duration_factor = 1 + (study_duration / 200)

    # Base strength in hours (typical initial forgetting ~24h)
    return 24 * diff_factor * score_factor * review_factor * duration_factor


def rule_based_retention(
...
def next_review_date(review_count: int, last_reviewed: datetime) -> datetime:
    """Returns the next scheduled review date based on interval index."""
...
async def ml_prediction(
    ...
    """
    Calls the ML microservice for a data-driven retention prediction.
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            return resp.json().get("retention_probability")
    except Exception:
        return None
