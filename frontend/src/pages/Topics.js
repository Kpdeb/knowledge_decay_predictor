...
import RetentionBadge, { retentionColor, retentionLabel } from "../components/RetentionBadge";
...
export default function Topics() {
  ...
  // Generate simulated decay curve for display
                  ...
                  <>
                    <div className="grid-3" style={{ marginBottom: 14 }}>
                      <div className="card-sm">
                        ...
                        <div className="stat-val" style={{ fontSize: 20, color: retentionColor(prediction.retention_rule_based) }}>
                          {Math.round(prediction.retention_rule_based * 100)}%
                        </div>
                      </div>
                      <div className="card-sm">
                        <div className="stat-label">ML Model</div>
                        <div className="stat-val" style={{ fontSize: 20 }}>
                          {prediction.retention_ml ? `${Math.round(prediction.retention_ml * 100)}%` : "N/A"}
                        </div>
                      </div>
                      <div className="card-sm">
                        <div className="stat-label">Next Review</div>
                        <div style={{ fontSize: 14, fontWeight: 600 }}>{prediction.next_review_days}d</div>
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: 12, padding: "10px 12px", background: "var(--bg3)", borderRadius: "var(--r)", fontSize: 12, color: "var(--text2)" }}>
                      {prediction.recommendation}
                    </div>
                  </>
                )}
                {!prediction && <div style={{ color: "var(--text3)", fontSize: 13 }}>Loading prediction…</div>}
              </div>
            )}
          </div>
        </div>
